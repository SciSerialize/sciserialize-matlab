%sciserialize_decoder parses a json string into Matlab data structures
% It is based on the parsejson function from the project transplant written
% by Bastian Bechtold (c) 2014.
% sciserialize_decoder(STRING)
...
%      - if the map contains a type, it is converted to the matlab
%      equivalent.
%
...
function [obj] = sciserialize_decoder(json)
...
function [obj, idx] = value(json, idx, str_tokens, num_tokens)
    ...
    if isa(obj , 'containers.Map') && isKey(obj, '__type__')   
